...
RUN cd /tmp && wget https://github.com/stevenlovegrove/Pangolin/archive/refs/tags/v0.6.zip \
    && unzip v0.6.zip && cd Pangolin-0.6 &&  mkdir build && cd build && cmake .. \
    && make -j4 && make install 

RUN cd /tmp && wget https://github.com/opencv/opencv/archive/refs/tags/3.2.0.zip -O opencv.zip \
    && unzip opencv.zip \
...
RUN cd /tmp && wget https://gitlab.com/libeigen/eigen/-/archive/3.3.9/eigen-3.3.9.zip \
    && unzip eigen-3.3.9.zip  && cd eigen-3.3.9 && mkdir build && cd build \
    && cmake -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=RELEASE .. \
    && make install \
    && rm -rf /tmp/*

WORKDIR /home
COPY ORB_SLAM3 /home/ORB_SLAM3
RUN cd ORB_SLAM3 && chmod +x build.sh && sh build.sh

RUN echo "export ROS_PACKAGE_PATH=/opt/ros/melodic/share:/home/ORB_SLAM3/Examples/ROS" >> /opt/ros/melodic/setup.bash \
&& echo "export ROS_PACKAGE_PATH=/opt/ros/melodic/share:/home/ORB_SLAM3/Examples/ROS" >> /$HOME/.bashrc \
&& /bin/bash -c "source /opt/ros/melodic/setup.bash \
&& source /$HOME/.bashrc \
&& cd ORB_SLAM3 && chmod +x build_ros.sh && sh build_ros.sh"
